...
class DDPG(DRL):
    ...
    def load(self, tag=""):
        ...
        if tag == "":
            ###uncomment below3 lines when you are using your weights
            #actor_file = "model/ddpg_actor.h5"
            #critic_Q_ex_file = "model/ddpg_critic_Q_ex.h5"
            #critic_Q_ex2_file = "model/ddpg_critic_Q_ex2.h5"

            # AUthors weight
            actor_file = "weights/bs/1month_every2day/ddpg_actor_5.h5"
            critic_Q_ex_file = "weights/bs/1month_every2day/ddpg_critic_Q_ex_5.h5"
            critic_Q_ex2_file = "weights/bs/1month_every2day/ddpg_critic_Q_ex2_5.h5"
            print("Auhtors weights are used........")
        else:
    ...
    def actor_optimizer(self):
       ...
       # x=trainable_weights
        ...
        params_grad = tf.gradients(ys=aoutput, xs=trainable_weights, grad_ys=-self.action_gradient)
...
if __name__ == "__main__":
    ...
    # def __init__(self, cash_flow_flag=0, dg_random_seed=1, num_sim=500002, sabr_flag = False, continuous_action_flag=False, spread=0, init_ttm=5, trade_freq=1, num_contract=1):
    env = TradingEnv(continuous_action_flag=True, sabr_flag=False, dg_random_seed=1, init_ttm=20, trade_freq=2, spread=0.01, num_contract=1, num_sim=50002)
    ddpg = DDPG(env)
    ...
    print("Training has to start.....")
    history = ddpg.train(60001)
    print("Training has been started.....")
    ddpg.save_history(history, "ddpg1monthevery2day60.csv")

    # setup for testing; use another instance for testing
    # def __init__(self, cash_flow_flag=0, dg_random_seed=1, num_sim=500002, sabr_flag = False, continuous_action_flag=False, spread=0, init_ttm=5, trade_freq=1, num_contract=1):
    env_test = TradingEnv(continuous_action_flag=True, sabr_flag=False, dg_random_seed=2, init_ttm=20, trade_freq=2, spread=0.01, num_contract=1, num_sim=100001)
    ddpg_test = DDPG(env_test)
    #tag = 49
    #Enable when RL Testing
    ddpg_test.load()

    # episode for testing: 0 to 100000 inclusive
    #test(self, total_episode, delta_flag=False, bartlett_flag=False):
